...
export async function sendWhatsAppMessage(phoneNumber, message, fromOverride = null) {
    ...
    // PRIORITIZE the configured Channel ID from environment if it's a 24-char YCloud ID (697...)
    // This is because YCloud usually requires the Channel ID for the 'from' field, not the phone number.
    let fromId = FROM_NUMBER;

    // Only if the env var is missing or empty, try the override
