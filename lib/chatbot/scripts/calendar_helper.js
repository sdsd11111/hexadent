...
export async function getAvailableSlots(dateStr, durationMin = 20, excludePhone = null) {
    try {
        ...
        console.log(`[Calendar Helper] Timing Check - Now (UTC): ${new Date(nowTs).toISOString()}, Lead Limit (UTC): ${new Date(leadLimitTs).toISOString()}`);
        ...
        // Returning empty allows logic.js to handle the retry/warning gracefully 
        // without inventing "optimistic" free slots which the user identified as risky.
        return [];
    }
}

export async function checkAvailability(date, time, durationMin = 20, excludePhone = null) {
    const slots = await getAvailableSlots(date, durationMin, excludePhone);
...
export async function bookAppointment(details) {
    const { name, phone, cedula, age, date, time, duration = 20, motive = "" } = details;
    const cleanPhone = phone.replace(/\D/g, '');
    const isAvail = await checkAvailability(date, time, duration, cleanPhone);
