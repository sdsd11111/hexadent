...
export async function getAvailableSlots(dateStr, durationMin = 45, excludePhone = null, isAdmin = false) {
    try {
        ...
        const stepMs = 45 * 60 * 1000; // CLEAN 45 MINUTE BLOCKS
        ...
        while (currentTs + durationMs <= limitTs) {
            ...
            // Lunch break check
            if (lunchHStart !== null) {
                const lunchStartTs = makeEcuTs(lunchHStart, lunchMStart);
                const lunchEndTs = makeEcuTs(lunchHEnd, 0);

                // Block if any part of the appointment falls inside the lunch break (13:15 - 15:00)
                if (currentTs < lunchEndTs && nextTs > lunchStartTs) {
                    isBlocked = true;
                }
            }
            ...
            // Admins still can't book in the past (before exact current time)
            if (isAdmin && currentTs < nowTs) {
                isBlocked = true;
            }
...
export async function bookAppointment(details, isAdmin = false) {
    let { name, phone = "", cedula, age, date, time, duration = 20, motive = "" } = details;
