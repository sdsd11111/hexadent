...
export async function getAvailableSlots(dateStr, durationMin = 20) {
    try {
        ...
        while (current.getTime() + requestedDurationMs <= limitDate.getTime()) {
            ...
            let inLunch = false;
            if (lunchStart !== null) {
                const endDecimalHour = (current.getTime() + requestedDurationMs) / (60 * 60 * 1000) - (startDay.getTime() / (60 * 60 * 1000)) + 9;
                // Simplified lunch check: if the start or end is within lunch
                if (decimalHour < lunchEnd && (decimalHour + durationMin / 60) > lunchStart) {
                    inLunch = true;
                }
            }
