...
export async function getAvailableSlots(dateStr, durationMin = 20) {
    try {
        ...
        if (!CALENDAR_ID) {
            console.error("[Calendar Helper] CRITICAL: GOOGLE_CALENDAR_ID is missing from environment variables.");
            return [];
        }

        // console.log(`[Calendar Helper] Querying ID: ${CALENDAR_ID} from ${startDay.toISOString()} to ${limitDate.toISOString()}`);
        ...
        // Use 15-minute increments for searching
        ...
        while (current.getTime() + requestedDurationMs <= limitDate.getTime()) {
            ...
            // Decimal hour calculation relative to the day's start to avoid timezone confusion
...
export async function checkAvailability(date, time, durationMin = 20) {
    try {
        ...
        if (dayOfWeek === 6 && time > "15:00") return false; // Saturday afternoon
        if (dayOfWeek === 6 && time < "08:30") return false; // Saturday morning
        if (time < "09:00" || (time > "13:00" && time < "15:00") || time > "18:00") {
...
export async function bookAppointment(details) {
    const { name, phone, cedula, date, time, duration = 20 } = details;
    ...
    try {
        ...
        if (dayOfWeek === 6 && time > "15:00") throw new Error("Los sábados solo atendemos hasta las 15:00.");
        if (dayOfWeek === 6 && time < "08:30") throw new Error("Los sábados atendemos desde las 08:30.");
        if (time < "09:00" || (time > "13:00" && time < "15:00") || time > "18:00") {
            if (dayOfWeek !== 6) throw new Error("Horario fuera de atención (09:00-13:00 o 15:00-18:00).");
        }
