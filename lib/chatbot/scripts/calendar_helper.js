...
export async function getAvailableSlots(dateStr, durationMin = 45, excludePhone = null, isAdmin = false) {
    try {
        // Use Ecuador timezone (-05:00) to avoid date shifting
        ...
        // BLOCKED DATE CHECK: Use DATE_FORMAT to ensure proper date comparison
        const [blocked] = await db.execute("SELECT reason FROM blocked_dates WHERE DATE_FORMAT(blocked_date, '%Y-%m-%d') = ?", [dateStr]);
        if (blocked.length > 0) {
        ...
        // Default: 9:00 to 18:30. Allowed last slot at 18:15 (ends 19:00)
...
export async function cancelAppointment(phone) {
    const cleanPhone = phone.replace(/\D/g, '');
    const [result] = await db.execute(
        'UPDATE appointments SET status = "cancelled" WHERE (patient_phone = ? OR patient_phone = ?) AND status != "cancelled" AND status != "completed" AND appointment_date >= CURDATE()',
        [phone, cleanPhone]
    );
    await db.execute('DELETE FROM chatbot_locked_slots WHERE locked_by = ? OR locked_by = ?', [phone, cleanPhone]);
