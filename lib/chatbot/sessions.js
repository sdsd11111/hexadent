...
export async function getSession(phone) {
    ...
    try {
        ...
        if (rows[0]) {
            // Normalize DATE (target_date)
            if (rows[0].target_date instanceof Date) {
                rows[0].target_date = rows[0].target_date.toISOString().split('T')[0];
            }
            // Normalize TIME (target_time)
            if (rows[0].target_time) {
                if (typeof rows[0].target_time === 'string') {
                    rows[0].target_time = rows[0].target_time.substring(0, 5);
                } else if (typeof rows[0].target_time === 'object') {
                    // MySQL2 TIME can be a Date object anchored to 1970-01-01
                    const d = rows[0].target_time;
                    if (d.getUTCHours) {
                        rows[0].target_time = `${d.getUTCHours().toString().padStart(2, '0')}:${d.getUTCMinutes().toString().padStart(2, '0')}`;
                    }
                }
            }
        }
