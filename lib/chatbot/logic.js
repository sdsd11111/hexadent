...
export async function processChatbotMessage(phoneNumber, messageText, clinicNumber = null) {
    ...
    try {
            ...
            if (hasActiveMotive) {
                ...
                if (isBlocked.length > 0 || (slots.length === 0 && targetDate !== '9999-99-99') || targetDate === dateStr || (new Date(`${targetDate}T00:00:00-05:00`) < earliestPossible)) {
                    ...
                    if (isBlocked.length > 0) {
                        availabilityContext = `[ESTADO: BLOQUEO TOTAL. La clínica está CERRADA el ${dayNameEcu.toUpperCase()} ${targetDate}. Motivo: ${reason}. NO PUEDES AGENDAR NADA PARA ESTE DÍA. No intentes buscar huecos ni menciones horarios de ese día.\nPRÓXIMOS DÍAS DISPONIBLES:${lookaheadStr}]`;
                    } else if (targetDate === dateStr || (new Date(`${targetDate}T00:00:00-05:00`) < earliestPossible)) {
                        availabilityContext = `[ESTADO: RECHAZO POR REGLA DE 24H. No se puede agendar para ${targetDate} porque hoy es ${dateStr} ${timeStr} (regla de 24h de anticipación). Explica esto al usuario con amabilidad.\nPRÓXIMOS DÍAS DISPONIBLES:${lookaheadStr}]`;
                    } else {
                        availabilityContext = `[ESTADO: DÍA LLENO. No hay espacio suficiente para una cita de ${duration} min el ${dayNameEcu.toUpperCase()} ${targetDate}. No sugieras horarios para este día.\nPRÓXIMOS DÍAS DISPONIBLES:${lookaheadStr}]`;
                    }
        ...
        2. **MOTIVO PRIMERO (CRÍTICO)**: Puedes dar información general (ej: "abrimos a las 9"), pero **ANTES de mostrar horarios disponibles específicos o confirmar una cita**, DEBES pedir el motivo para calcular el tiempo (Limpieza 20m, Ortodoncia 45m, Niños 60m).
        3. **VERIFICACIÓN DE SLOTS (REGLA DE ORO)**: NO CONFÍES en tu memoria. SÓLO puedes sugerir horarios de la "LISTA DE HORARIOS DISPONIBLES". Si un horario como las 12:40 aparece en la lista para una cita de 20 min, ¡ENTONCES SÍ SE PUEDE! El sistema ya calculó que termina justo a la 1:00.
        4. **CONCORDANCIA DE TIEMPO**: Si un usuario pide una hora (ej: 14:30 para niño el sábado), búscarla en la lista dinámica. Si NO está exactamente igual, DI QUE NO HAY TIEMPO SUFICIENTE para esa duración y ofrece otros horarios de la lista.
        5. **SÁBADOS (LÍMITE REAL)**: Los sábados cerramos a las 15:00. Cualquier cita de tarde debe terminar ANTES de la hora de cierre. Una cita de 20 min a las 14:40 es válida. Confía totalmente en lo que aparezca en la lista de slots.
        6. **REGLA DE LAS 24 HORAS**: Por política de la clínica, solo se pueden agendar citas con al menos 24 HORAS DE ANTICIPACIÓN. Hoy es ${dayEcu} ${dateStr} a las ${timeStr}.
        7. **DURACIÓN**: La duración depende del motivo (Limpieza/Calza: 20m, Ortodoncia Control: 45m, Niños: 60m). El sistema calcula esto automáticamente; tú solo obedece la disponibilidad que le entrega.
