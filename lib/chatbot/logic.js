...
export async function processChatbotMessage(phoneNumber, messageData, clinicNumber = null) {
    ...
    try {
        ...
        // 1. Primero: Limpiar caracteres inválidos y ? sueltos que NO son puntuación válida
        // Detectar ? que está entre palabras (no al final) y parece error de encoding
        finalResponseText = finalResponseText.replace(/([a-záéíóúñA-ZÁÉÍÓÚÑ])\?([a-záéíóúñA-ZÁÉÍÓÚÑ])/gi, '$1$2');  // ? entre letras -> quitar
        
        // 2. Detectar si hay preguntas sin cerrar y corregir ANTES de manejar emojis
        // Si tiene ¿ pero no termina con ?, añadir ?
        if (finalResponseText.includes('¿') && !finalResponseText.trim().endsWith('?')) {
            // Verificar que la última oración con ¿ no tenga ?
        ...
        // 3. Detectar preguntas que terminan con ? pero no empiezan con ¿
        const questionWords = /\b(en\s+qué|cómo|cuándo|dónde|por\s+qué|quién|cuál|cuáles|cuánto|cuánta|cuántos|tiene|podría|desea|necesita|le\s+gustaría|está|hay|puede|prefiere|es|sería)\b/i;
        if (questionWords.test(finalResponseText) && finalResponseText.trim().endsWith('?') && !finalResponseText.includes('¿')) {
            finalResponseText = '¿' + finalResponseText;
            console.log("[Logic] Added opening question mark.");
        }
        
        // 4. Ahora manejar ? sueltos que parecen emojis (pero solo si NO son puntuación válida)
        // Si termina con ? pero NO tiene ¿, reemplazar con emoji
        if (!finalResponseText.includes('¿') && finalResponseText.trim().endsWith('?')) {
        ...
        // El emoji debe ir ANTES del ? cuando es al final de pregunta
        finalResponseText = finalResponseText.replace(/\?\s*😊/g, ' 😊 ?');  // Mover emoji antes del ?
        finalResponseText = finalResponseText.replace(/\?\s*👍/g, ' 👍 ?');  // Mover emoji antes del ?
        
        // 7. Si termina con ? pero sin emoji, añadir uno amigable
