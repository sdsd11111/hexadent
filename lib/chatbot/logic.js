...
export async function processChatbotMessage(phoneNumber, messageText, clinicNumber = null) {
    ...
    try {
        ...
        if (lowerMsg.includes('cita') || lowerMsg.includes('disponibilidad') || lowerMsg.includes('cuándo') || lowerMsg.includes('horario') || lowerMsg.includes('cuanto') || lowerMsg.includes('hay') || lowerMsg.includes('se puede') || lowerMsg.includes('tienes espacio') || lowerMsg.includes('puedo ir')) {
            ...
            // Explicit guard for the AI: tell it what to say if the requested time is invalid
            ...
            let timeWarning = "";
            if (requestedTimeMatch && isSaturday) {
                let hour = parseInt(requestedTimeMatch[1]);
                const pm = requestedTimeMatch[3]?.toLowerCase().includes('p');
                if (pm && hour < 12) hour += 12;
                if (hour >= 14 || (hour === 13 && parseInt(requestedTimeMatch[2] || "0") > 30)) {
                    timeWarning = "\n⚠️ ERROR: El usuario pregunta por la tarde de un sábado. DI QUE EL SÁBADO CERRAMOS A LAS 13:30.";
                }
            }

            availabilityContext = `LISTA DE HORARIOS DISPONIBLES PARA EL ${targetDate} (${messageText}): ${slots.length > 0 ? slots.join(', ') : 'No hay turnos disponibles para este día.'}
...
${getReferenceCalendar()}
        ...
        - HORARIOS DE ATENCIÓN (ESTRICTO):
            * Lunes a Viernes: 09:00 - 13:00 y 15:00 - 18:30 (Cerrado 13:00-15:00)
            * Sábados: 09:00 - 13:30 (Cerrado tarde)
            * Domingos: CERRADO
        ...
        5. DISPONIBILIDAD ESTRICTA: NUNCA confirmes una cita en un horario que no esté listado en el context. Si el context dice "No hay turnos", ofrece otro día. Sábados solo hasta 13:30.
        ...
        if (cleanBotResponse && cleanBotResponse.length > 0) {
