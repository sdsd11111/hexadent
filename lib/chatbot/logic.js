...
export async function processChatbotMessage(phoneNumber, messageData, clinicNumber = null) {
    ...
    try {
            ...
            if (hasActiveMotive && targetDate) {
                try {
                    ...
                    if (isBlockedRows.length > 0) {
                        ...
                        availabilityContext = `⚠️ NO HAY CUPOS DISPONIBLES para ${dayName} ${targetDate} (respetando la regla de 8h). Ofrezca MAÑANA u otro día.`;
                    }
        ...
        const systemPrompt = `
            ...
            REGLA DE ORO: Si NO hay horarios en la lista de arriba para hoy, es porque NO SE PUEDE agendar para hoy (Regla de 8h). No inventes horarios.
            REGLA DE ORO: Si el usuario ya te dio el motivo (ej: dolor, limpieza), NO lo preguntes de nuevo. Procede directamente con la disponibilidad.
            USA FORMATO HORIZONTAL (comas) para horarios. Muestra TODOS los horarios (ej: 09:00, 09:15, 09:30...).
