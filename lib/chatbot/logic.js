    ...
    try {
        ...
        if (!lowerMsg.includes('debug-reveal') && (isExplicitBookingIntent || isDateConfirmation || isCurrentlyInBookingFlow)) {
            ...
            if (hasActiveMotive && targetDate && targetDate !== '9999-99-99' && slots.length === 0) {
                availabilityContext += `\n⚠️ AVISO LEGAL DE DISPONIBILIDAD: EL DÍA ${targetDate} ESTÁ TOTALMENTE LLENO O CERRADO. ESTÁ TERMINANTEMENTE PROHIBIDO SUGERIR CUALQUIER HORA. DI QUE TODO EL DÍA ESTÁ LLENO O CERRADO Y OFRECE OTRO DÍA.`;

                history.push({
                    role: "system",
                    content: `🔴 ALERTA DE SISTEMA: El motor de reservas confirma que NO HAY DISPONIBILIDAD para el ${targetDate}. 
                    NO des horarios inventados. 
                    DI LITERALMENTE: "Lamentablemente no tenemos turnos disponibles para esa fecha".`
                });
            }
        ...
        **IDENTIDADDEL PACIENTE:**
        ...
        ⚠️ REGLA DE EXHIBICIÓN LITERAL:
        - DEBES mostrar la lista de horarios EXACTAMENTE como se te entrega.
        - PROHIBIDO resumir (ej: no digas "de 9 a 11" si la lista dice "09:00, 09:15, 09:30...").
        - PROHIBIDO redondear a horas cerradas (ej: no omitas los :15, :30, :45).
        - SIEMPRE utiliza una lista con viñetas (-) para mostrar los horarios.
