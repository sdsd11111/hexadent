...
export async function processChatbotMessage(phoneNumber, messageData, clinicNumber = null) {
    ...
    if (!messageText) return;
                        ...
                        if (targetDate > maxStrLogic) {
                            ...
                            if (dateTruth.includes('Cerrado')) {
                                ...
                                if (isBlockedRows.length > 0) {
                                    availabilityContext = `[ESTADO: DÍA BLOQUEADO para ${targetDate}. Motivo: ${isBlockedRows[0].reason || 'La clínica estará cerrada'}. ${dateTruth}. IMPORTANTE: NO uses palabras técnicas como "bloqueado en la agenda". Dile al usuario de forma natural que "No tenemos disponibilidad para ese día".]`;
                                } else {
                                    availabilityContext = `⚠️ NO HAY CUPOS para ${targetDate}. ${dateTruth}`;
                                }
                            } else if (slots.length > 0) {
                                let displaySlots = slots.filter(s => s !== "14:00" && s !== "16:00" && s !== "08:00");
                                availabilityContext = `⚠️ HORARIOS REALES DISPONIBLES (${targetDate}): ${displaySlots.join(', ')}. ${dateTruth}`;
                            }
                            toolExecutionResult = availabilityContext;
                        }
                    }
        ...
        const systemPrompt = `
            ...
            REGLA DE FORMATO DE HORARIOS TARDE: Para horarios de la tarde (15:00-18:00), está PROHIBIDO listar cada slot individual si hay muchos seguidos. En su lugar, menciona el rango disponible basado en los horarios REALES que ves arriba (ej: si los horarios libres empiezan a las 15:45, di "Tenemos disponibilidad en la tarde a partir de las 15:45" o "de 15:45 a 17:30"). NUNCA ofrezcas un rango que empiece en un horario que NO esté en la lista de arriba.
