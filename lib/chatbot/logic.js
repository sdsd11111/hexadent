...
export async function processChatbotMessage(phoneNumber, messageData, clinicNumber = null) {
    ...
    // RESTRICTED TEST MODE (lib/logic) - SOLO responde a este número
    const allowedNumber = '593983237491';
    if (!cleanPhone.endsWith('983237491') && cleanPhone !== allowedNumber) {
        console.log(`[Chatbot Logic] RESTRICTED MODE. BLOCKED message from ${cleanPhone}`);
        return;
    }
    ...
    try {
        ...
        if (bestMatch || proposedTime) {
            ...
            if (isTargetSaturday) {
                // Sábados: 08:30 a 15:00 (sin cierre al mediodía)
                const timeInMinutes = finalH * 60 + finalM;
                isWithinBusinessHours = timeInMinutes >= (8 * 60 + 30) && timeInMinutes < (15 * 60);
            ...
            if (!isWithinBusinessHours && targetDate) {
                let hoursMessage = isTargetSaturday 
                    ? "los sábados atendemos de 08:30 a 15:00" 
                    : "de lunes a viernes atendemos de 09:00 a 13:00 y de 15:00 a 18:00";
                availabilityContext = `[ERROR CRÍTICO DE HORARIO: ${finalTime} está FUERA DEL HORARIO DE ATENCIÓN para ${targetDate}. ${hoursMessage}. ESTÁ PROHIBIDO agendar a esta hora. DEBES decirle al usuario amablemente que ese horario no está disponible y pedirle que elija otro dentro del horario permitido. NUNCA digas que es un "buen horario" ni pidas datos personales si el horario es inválido.]`;
        ...
        const systemPrompt = `
            ...
            REGLA DE FORMATO DE HORARIOS TARDE: Para horarios de la tarde (15:00-18:00), está PROHIBIDO listar cada slot individual (15:00, 15:15, 15:30...). En su lugar di: "Tenemos disponibilidad de 15:00 a 17:30. ¿Algún horario en ese rango le funciona?" Solo si el usuario pide específicamente la lista, entonces sí la das.
            ...
            - Lunes-Viernes: ÚNICAMENTE 09:00 a 13:00 y 15:00 a 18:00. NADA fuera de esos rangos.
            - Sábados: ÚNICAMENTE 08:30 a 15:00. NADA después de las 15:00.
            ...
            Muestra UN día de ejemplo (ej: "De lunes a viernes tenemos: 09:00, 09:20, ...") y menciona el sábado por separado. SOLO lista días adicionales de forma individual si tienen huecos OCUPADOS que los diferencien del resto. El objetivo es que el paciente lea poco pero entienda todo.
        ...
        if (isFirstMessage) {
            ...
            if (!finalResponseText.startsWith(requiredGreeting)) {
                // Si el LLM no puso el saludo correcto, lo forzamos al inicio
                console.log(`[Logic] WARNING: First message without proper greeting. Forcing prefix. Raw: ${finalResponseText.substring(0, 50)}...`);
                finalResponseText = `${requiredGreeting} ${finalResponseText.replace(/^Hola[\s,!]*/i, '').trim()}`;
            } else {
                console.log("[Logic] First message greeting VALIDATED.");
            }
        }

        // LIMPIEZA DE PUNTUACIÓN Y CARACTERES INVÁLIDOS
        // 1. Reemplazar "?" sueltos (errores de encoding donde debería haber emoji) con emojis apropiados
        finalResponseText = finalResponseText.replace(/\?\s*$/g, ' 😊');  // ? al final -> emoji sonrisa
        finalResponseText = finalResponseText.replace(/\?\s+(?=[A-ZÁÉÍÓÚ¿])/g, ' 👍 ');  // ? entre frases -> pulgar arriba
        finalResponseText = finalResponseText.replace(/([a-záéíóúñ])\?(\s)/gi, '$1$2');  // ? después de palabra (no pregunta) -> quitar
        
        // 2. Corregir preguntas que empiezan con ¿ pero no terminan con ?
        const spanishQuestionWords = /¿(en\s+qué|cómo|cuándo|dónde|por\s+qué|quién|cuál|cuánto|cuánta|tiene|podría|desea|le\s+gustaría|está|hay|puede)/i;
        if (spanishQuestionWords.test(finalResponseText) && !finalResponseText.trim().endsWith('?')) {
            // Buscar la última oración que empieza con ¿ y asegurar que termine con ?
            const lastQuestionMatch = finalResponseText.match(/¿[^¿]+$/);
            if (lastQuestionMatch && !lastQuestionMatch[0].includes('?')) {
                finalResponseText = finalResponseText.trim() + '?';
                console.log("[Logic] Fixed unclosed question mark.");
            }
        }
        
        // 3. Corregir preguntas que terminan con ? pero no empiezan con ¿ (cuando hay palabras interrogativas)
        const hasQuestionWords = /\b(en\s+qué|cómo|cuándo|dónde|por\s+qué|quién|cuál|cuánto|cuánta|tiene|podría|desea|le\s+gustaría|está|hay|puede)\b/i;
        if (hasQuestionWords.test(finalResponseText) && finalResponseText.trim().endsWith('?') && !finalResponseText.includes('¿')) {
            finalResponseText = '¿' + finalResponseText;
            console.log("[Logic] Added opening question mark.");
        }
