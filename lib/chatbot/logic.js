...
export async function processChatbotMessage(phoneNumber, messageText, clinicNumber = null) {
    ...
    try {
                ...
                if (isBlocked.length > 0 || (slots.length === 0 && targetDate !== '9999-99-99')) {
                    ...
                    for (let i = 1; i <= 14 && found < 2; i++) {
                        ...
                        if (s.length > 0) {
                            ...
                            const morningCount = s.filter(t => parseInt(t.split(':')[0]) < 13).length;
                            const afternoonCount = s.filter(t => parseInt(t.split(':')[0]) >= 15 || (dName.toLowerCase().includes('sábado') && parseInt(t.split(':')[0]) >= 12)).length;

                            let periods = [];
                            if (morningCount > 0) periods.push("MAÑANA");
                            if (afternoonCount > 0) periods.push("TARDE");

                            lookaheadStr += `\n- ${dName.toUpperCase()} ${dStr}: Disponible en la ${periods.join(' y ')}.`;
                            found++;
                        }
                    }
        ...
        3. **VERIFICACIÓN DE SLOTS (REGLA DE ORO)**: NO CONFÍES en tu conocimiento general de los horarios. SÓLO puedes sugerir los horarios que aparecen en la "LISTA DE HORARIOS DISPONIBLES". Si la lista está vacía o el día está bloqueado, TIENES PROHIBIDO decir "podemos a tal hora" (ni siquiera a las 14:30 de un sábado si no está en la lista).
        4. **CONCORDANCIA DE TIEMPO**: Si un usuario pide una hora (ej: 14:30 para niño), búscala en la lista dinámica. Si NO está en la lista (porque la cita es larga y no cabe), DI QUE NO HAY TIEMPO SUFICIENTE para esa duración y ofrece otros horarios de la lista.
