    ...
    try {
        ...
        if (!lowerMsg.includes('debug-reveal') && (lowerMsg.includes('cita') || lowerMsg.includes('disponibilidad') || lowerMsg.includes('cuándo') || lowerMsg.includes('horario') || lowerMsg.includes('cuanto') || lowerMsg.includes('hay') || lowerMsg.includes('se puede') || lowerMsg.includes('tienes espacio') || lowerMsg.includes('puedo ir') || lowerMsg.includes('agendar') || lowerMsg.includes('reservar') || lowerMsg.includes('agenda') || lowerMsg.includes('reserva'))) {
            ...
            if (isBlocked.length > 0) {
                ...
                availabilityContext = `[BLOQUEO ABSOLUTO: LA CLÍNICA ESTÁ CERRADA EL ${dayNameEcu.toUpperCase()} ${targetDate}. Motivo: ${reason}. NO PUEDES AGENDAR NADA PARA ESTE DÍA. Comunícalo al paciente y ofrece el día laborable más cercano.]`;
        ...
        1. **Cédula**: DEBE tener exactamente 10 dígitos. Si el usuario da menos (como 8), NO confirmes la cita; pide amablemente los 10 dígitos.
        ...
        3. **Bloqueos**: Si el sistema dice [BLOQUEO ABSOLUTO], es PROHIBIDO agendar. No menciones horarios.
        ...
        if (!bookingExecuted && metadata && metadata.action === 'create_appointment') {
            ...
            if (!mName || !mCedula || !mAge || mCedula === "Pendiente") {
                ...
                validationError = "La cédula debe tener exactamente 10 dígitos.";
