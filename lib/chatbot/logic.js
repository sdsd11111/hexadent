...
export async function processChatbotMessage(phoneNumber, messageText, clinicNumber = null) {
    ...
    try {
        ...
        if (lowerMsg.includes('cita') || lowerMsg.includes('disponibilidad') || lowerMsg.includes('cuándo') || lowerMsg.includes('horario') || lowerMsg.includes('cuanto') || lowerMsg.includes('hay') || lowerMsg.includes('se puede') || lowerMsg.includes('tienes espacio') || lowerMsg.includes('puedo ir')) {
            ...
            if (isGeneral && !isOrtho && !isPediatric) duration += 20; // Only add if not already covered by ortho/pedia
            if (duration === 0) duration = 20; // Default
            ...
            // Calculate the last possible start time for this duration
            const limitHour = isSaturday ? 15 : 18;
            const lastStartMinutes = (limitHour * 60) - duration;
            const lastH = Math.floor(lastStartMinutes / 60);
            const lastM = lastStartMinutes % 60;
            ...
            ⚠️ REGLA DE ORO DE HORARIOS: La clínica CIERRA TOTALMENTE a las ${limitHour}:00. 
            - Los doctores se van a las ${limitHour}:00. 
            - Como el tratamiento dura ${duration} min, la ÚLTIMA cita posible debe empezar a las ${lastTimeStr}. 
            - NUNCA menciones las ${limitHour}:00 como opción de inicio.
            - Si el horario que pide el usuario (ej: 14:30) está en la LISTA de arriba, ACÉPTALO. No inventes que la última es a las ${limitHour}:00.${timeWarning}`;
