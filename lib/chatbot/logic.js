                ...
                try {
                    ...
                    if (targetDate < todayStrLogic) {
                        ...
                        if (targetDate > maxStrLogic) {
                            availabilityContext = `[ESTADO: FECHA MUY LEJANA. El ${targetDate} supera nuestro límite de 6 meses. Pídele al usuario una fecha más cercana (máximo hasta ${maxStrLogic}).]`;
                            toolExecutionResult = availabilityContext;
                        } else {
                            ...
                            if (dateTruth.includes('Cerrado')) {
                                ...
                                if (isBlockedRows.length > 0) {
                                    availabilityContext = `[ESTADO: DÍA BLOQUEADO para ${targetDate}. Motivo: ${isBlockedRows[0].reason || 'La clínica estará cerrada'}. ${dateTruth}. Dile al usuario que ese día no atendemos y ofrece otro día.]`;
                                } else {
