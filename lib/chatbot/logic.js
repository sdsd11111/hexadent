    ...
    if (!messageText) return;
        ...
        if (isSimpleConfirmation) {
            ...
            try {
                ...
                // Search WITHOUT status filter to find appointments for this phone
                ...
                // Search ALL appointments for this phone (no date filter)
                const [appts] = await db.execute(
                    `SELECT id, appointment_date, appointment_time, patient_name, motive, status FROM appointments 
                     WHERE (patient_phone LIKE ? OR patient_phone = ? OR patient_phone = ? OR patient_phone = ? OR patient_phone = ? OR patient_phone = ?)
                     ORDER BY appointment_date ASC, appointment_time ASC LIMIT 10`,
                    searchTerms
                );
                
                console.log(`[Logic] DEBUG: Found ${appts.length} appointments (any date, any status)`);
