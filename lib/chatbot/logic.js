...
export async function processChatbotMessage(phoneNumber, messageText, clinicNumber = null) {
    ...
    try {
        ...
        if (!lowerMsg.includes('debug-reveal') && (lowerMsg.includes('cita') || lowerMsg.includes('disponibilidad') || lowerMsg.includes('cuándo') || lowerMsg.includes('horario') || lowerMsg.includes('cuanto') || lowerMsg.includes('hay') || lowerMsg.includes('se puede') || lowerMsg.includes('tienes espacio') || lowerMsg.includes('puedo ir') || lowerMsg.includes('agendar') || lowerMsg.includes('reservar') || lowerMsg.includes('agenda') || lowerMsg.includes('reserva'))) {
            ...
            const isPediatric = lowerMsg.includes('niño') || lowerMsg.includes('niña') || lowerMsg.includes('hijo') || lowerMsg.includes('hija') || lowerMsg.includes('bebé') || lowerMsg.includes('bebe') || lowerMsg.includes('pequeñ') || lowerMsg.includes('infantil');
        ...
        const getReferenceCalendar = () => {
            ...
            lines.push(`\nREGLA: NUNCA ofrezcas un DOMINGO. Están cerrados.`);
            return lines.join('\n');
        };
        ...
        5. **Veracidad de Calendario**: NUNCA inventes qué día de la semana es una fecha. MIRA el "CALENDARIO DE REFERENCIA" abajo y COPIA el nombre del día literalmente. Si dices algo como "25 de febrero es jueves" y en la lista dice "miércoles", estarás cometiendo un error grave.
        ...
        2. **MOTIVO PRIMERO**: NUNCA muestres horarios, ni digas "estamos libres tales días", ni sugieras fechas si aún no sabes el MOTIVO de la consulta.
        3. **VERIFICACIÓN DE SLOTS**: Si el usuario pide una hora específica (ej: "17:30"), DEBES buscarla en la "LISTA DE HORARIOS DISPONIBLES" de ese día. Si NO está en la lista, RESPONDE QUE NO ES POSIBLE y ofrece las alternativas que sí están en la lista.
        4. **REGLA DE LAS 24 HORAS**: Por política de la clínica, solo se pueden agendar citas con al menos 24 HORAS DE ANTICIPACIÓN. Si hoy es miércoles 12:00, lo más pronto es jueves 12:00.
        ...
        9. **FORMATO DE LISTA**: Presenta los horarios en bloques horizontales usando el separador "|" (ej: 09:00 | 09:30 | 10:00).
