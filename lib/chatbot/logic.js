...
export async function processChatbotMessage(phoneNumber, messageData, clinicNumber = null) {
    ...
    try {
        ...
        if (bestMatch || proposedTime) {
            ...
            if (slots.includes(finalTime) || (targetDate && new Date(targetDate).getDay() === 6 && finalTime === "08:30")) {
                session.target_time = finalTime;
                await lockSlot(cleanPhone, targetDate, finalTime);
                availabilityContext += `\n[SLOT VERIFICADO: ${finalTime} está DISPONIBLE. Pide datos personales Nombre/Cédula/Edad.]`;
            } else {
                availabilityContext += `\n[SLOT NO DISPONIBLE: ${finalTime} no está libre.]`;
