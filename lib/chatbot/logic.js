                ...
                if (scheduledAppts.length > 0) {
                    ...
                    const formatDate = (d) => {
                        ...
                        try {
                            let dateObj;
                            if (typeof d === 'string') {
                                const dateStr = d.split('T')[0]; // Handle "2026-06-22T00:00:00.000Z" format
                                dateObj = new Date(dateStr + 'T12:00:00-05:00');
                            ...
                            return String(d).split('T')[0]; // Fallback to just date string
                        }
                    };
                    
                    // Update status to confirmed
                    console.log(`[Logic] Updating appointment ${scheduledAppts[0].id} to confirmed...`);
                    await db.execute(
                        'UPDATE appointments SET status = "confirmed" WHERE id = ?',
                        [scheduledAppts[0].id]
                    );
                    console.log(`[Logic] Update complete for id ${scheduledAppts[0].id}`);
