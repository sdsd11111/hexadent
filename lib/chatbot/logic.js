    ...
    try {
        ...
        if (bestMatch || proposedTime) {
            ...
            if (slots.includes(finalTime) || (isTargetSaturday && finalTime === "08:30")) {
                ...
                availabilityContext += `\n[SLOT VERIFICADO: ${finalTime} está DISPONIBLE para el ${targetDate}. Pide datos personales Nombre/Cédula/Edad o confirma la reserva.]`;
            } else {
        ...
        if (!metadata && (botResponse.toLowerCase().includes('agendada') || botResponse.toLowerCase().includes('confirmada'))) {
            console.log(`[Logic] No metadata but bot said "confirmed". Attempting session fallback...`);
            if (session.target_date && session.target_time && session.cedula) {
        ...
        if (metadata && metadata.action === 'create_appointment' && !bookingExecuted) {
            ...
            if (!metadata.name || !metadata.cedula || !validateCedula(metadata.cedula)) {
                console.log(`[Logic] Missing or invalid patient data in metadata. Name: ${metadata.name}, Cedula: ${metadata.cedula}`);
                ...
                console.log(`[Booking Attempt] Date: ${finalDate}, Time: ${finalTime}, Patient: ${metadata.name}`);

                const booking = await bookAppointment({ ...metadata, date: finalDate, time: finalTime, phone: cleanPhone });
                if (booking.success) {
