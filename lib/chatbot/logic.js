...
export async function processChatbotMessage(phoneNumber, messageText, clinicNumber = null) {
    ...
    try {
        ...
        let patientContext = session ? `- Paciente: ${session.name} (Cédula: ${session.cedula}, Edad: ${session.age || 'No proporcionada'})` : "- Paciente: No identificado aún";
        ...
        if (!lowerMsg.includes('debug-reveal') && (lowerMsg.includes('cita') || lowerMsg.includes('disponibilidad') || lowerMsg.includes('cuándo') || lowerMsg.includes('horario') || lowerMsg.includes('cuanto') || lowerMsg.includes('hay') || lowerMsg.includes('se puede') || lowerMsg.includes('tienes espacio') || lowerMsg.includes('puedo ir') || lowerMsg.includes('agendar') || lowerMsg.includes('reservar') || lowerMsg.includes('agenda') || lowerMsg.includes('reserva'))) {
            ...
            if (isBlocked.length > 0) {
                ...
                availabilityContext = `[ESTADO CRÍTICO: LA CLÍNICA ESTARÁ CERRADA EL ${dayNameEcu.toUpperCase()} ${targetDate}. Motivo: ${reason}. Informa al paciente con mucha amabilidad que ese día no habrá atención y ofrece el día laborable más cercano.]`;
        ...
        3. **PRESENTACIÓN**: No muestres horarios hasta que sepas el "Motivo" y si es "Adulto/Niño".
        4. **DOMINGOS Y HORAS**: Estamos CERRADOS los domingos. Sábados solo hasta las 15:00. NUNCA ofrezcas horarios fuera de los proporcionados por el sistema.
        5. **FORMATO DE LISTA**: Presenta los horarios en bloques horizontales usando el separador "|" (ej: 09:00 | 09:30 | 10:00). NO hagas listas largas hacia abajo.
        ...
        if (!bookingExecuted && metadata && metadata.action === 'create_appointment') {
            ...
            if (!mName || !mCedula || !mAge) {
                ...
                bookingExecuted = true;
            } else if (mAge > 150 || mAge < 0) {
                toolExecutionResult = `[SISTEMA: ERROR - Edad inválida (${mAge}). Por favor verifíquela.]`;
                bookingExecuted = true;
            } else if (mIsPediatric && mAge > 12) {
                toolExecutionResult = `[SISTEMA: ADVERTENCIA - Estás agendando como "niño" a alguien de ${mAge} años. Confirma si realmente es pediátrico o si es un adulto.]`;
                bookingExecuted = true;
            } else if (!mIsPediatric && mAge < 12) {
                toolExecutionResult = `[SISTEMA: ADVERTENCIA - Estás agendando como "adulto" a un niño de ${mAge} años. Por favor confirma o cambia a cita infantil.]`;
                bookingExecuted = true;
            } else {
        ...
        if (!bookingExecuted) {
            const partialName = metadata?.name || session?.name;
            const partialCedula = metadata?.cedula || session?.cedula;
            const partialAge = metadata?.age || session?.age;

            // Limit flow description
            let flowStatus = "";
            if (partialName && !partialCedula) flowStatus = "Esperando Cédula";
            else if (partialCedula && !partialAge) flowStatus = "Esperando Edad";
            else if (partialAge && !metadata?.time) flowStatus = "Esperando Horario";

            await saveSession(cleanPhone, {
                name: partialName,
                cedula: partialCedula,
                age: partialAge,
                current_flow: flowStatus,
                metadata: metadata
            });
        }
