...
export async function processChatbotMessage(phoneNumber, messageText, clinicNumber = null) {
    ...
    try {
        ...
        2. Al tener todo, AGENDA Y CONFIRMA usando el bloque [METADATA: ...] al final.
        `;
        ...
        if (!metadata && (botResponse.toLowerCase().includes('agendado') || botResponse.toLowerCase().includes('confirmado'))) {
            ...
            if (session && session.target_date && session.target_time) {
                metadata = {
                    ...
                    cedula: session.patient_cedula || "0",
        ...
        if (finalResponseText && finalResponseText.length > 0) {
            // DIAGNOSTIC: Append tool execution result for developer to see in screenshot
            const diagnosticTag = `\n\n[DEBUG: ${toolExecutionResult || "NONE"}]`;
            await sendEvolutionWhatsApp(phoneNumber, finalResponseText + diagnosticTag);
        } else {
