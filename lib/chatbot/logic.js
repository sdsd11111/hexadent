...
export async function processChatbotMessage(phoneNumber, messageData, clinicNumber = null) {
    ...
    try {
        ...
        if (lowerMsg.includes('limpiar-todo-hexadent-ahora')) {
            console.log(`[Chatbot Logic] NUCLEAR RESET TRIGGERED BY ${cleanPhone}`);
            ...
            await db.execute('DELETE FROM chatbot_locked_slots');
            await db.execute('DELETE FROM handoff_sessions');
            await sendEvolutionWhatsApp(phoneNumber, "✅ Memoria total del sistema borrada con éxito. El bot está como nuevo. 😊🦷");
            return { success: true };
        }
        ...
        if (!lowerMsg.includes('debug-reveal') && (isExplicitBookingIntent || isDateConfirmation || isCurrentlyInBookingFlow)) {
            ...
            if (finalResolved && finalResolved !== session?.target_date) {
                await sessions.updateSession(phone, { target_date: finalResolved });
            }
