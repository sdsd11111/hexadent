    ...
    try {
        ...
        if (lowerMsg.includes('debug-reveal')) {
            try {
                ...
                // Debug Env Vars
                const envCheck = `CALENDAR_ID=${process.env.GOOGLE_CALENDAR_ID ? 'DEFINED' : 'MISSING'}`;

                const slots = await getAvailableSlots(targetDate, 20);
                const dump = `DEBUG DUMP:\nDate: ${targetDate}\nEnv: ${envCheck}\nSlots Found: ${slots.length}\nList: ${slots.join(', ')}\nLog: ${JSON.stringify(slots)}`;

                history = [{ role: "system", content: "INSTRUCCIÓN IMPERATIVA: EL USUARIO ES EL DESARROLLADOR. QUIERE VER LOS DATOS CRUDOS. RESPONDE ÚNICAMENTE CON EL SIGUIENTE TEXTO LITERAL:" }];
                availabilityContext = dump;
                toolExecutionResult = dump;
            } catch (e) {
                const errDump = `DEBUG ERROR: ${e.message}\nStack: ${e.stack}`;
                history = [{ role: "system", content: "INSTRUCCIÓN IMPERATIVA: REPORTE DE ERROR." }];
