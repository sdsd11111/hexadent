    ...
    if (!messageText) return;
        ...
        if (isSimpleConfirmation) {
            ...
            try {
                ...
                // Search WITHOUT status filter to find appointments for this phone - include ALL future appointments
                ...
                // First try to find today's appointment
                ...
                console.log(`[Logic] DEBUG: Found ${todayAppts.length} appointments for TODAY`);
                
                // If no today appointments, search all future appointments
                const [appts] = todayAppts.length > 0 ? [todayAppts] : await db.execute(
                    `SELECT id, appointment_date, appointment_time, patient_name, motive, status FROM appointments 
                ...
                console.log(`[Logic] DEBUG: ${scheduledAppts.length} scheduled appointments`);
                
                // If no scheduled, check for confirmed appointments (already confirmed)
        ...
        if (isCancellation) {
            ...
            try {
                ...
                console.log(`[Logic] DEBUG CANCEL: clean8=${clean8}, withZero=${withZero}, with593=${with593}`);
                ...
                const [appts] = await db.execute(
                    `SELECT id, appointment_date, appointment_time FROM appointments 
                     ...
                     AND (patient_phone LIKE ? OR patient_phone = ? OR patient_phone = ? OR patient_phone = ? OR patient_phone = ?)
                     LIMIT 1`,
                    ['%' + clean8 + '%', withZero, clean8, with593, '0' + clean8]
                );
                
                console.log(`[Logic] DEBUG CANCEL: Found ${appts.length} appointments (any status)`);
                
                // Filter to only scheduled appointments
                const scheduledAppts = appts.filter(a => a.status === 'scheduled');
                console.log(`[Logic] DEBUG CANCEL: ${scheduledAppts.length} scheduled appointments`);
                
                if (scheduledAppts.length > 0) {
                    console.log(`[Logic] CANCELLATION: Found appointment to cancel, ID: ${scheduledAppts[0].id}`);
