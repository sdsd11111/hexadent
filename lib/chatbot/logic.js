...
// validateCedula no longer needed for booking flow
...
export async function processChatbotMessage(phoneNumber, messageData, clinicNumber = null) {
    ...
    if (!messageText) return;
        ...
        // NOTE: Cedula/age detection removed - only name is required for booking
        ...
        if (isAwaitingConfirmation) {
            ...
            const isNegative = /^(\s*no\s*|\s*cancelar\s*|\s*no\s*asistire\s*|\s*no\s*podre\s*)$/i.test(lowerMsg) || 
                               ...
                               (lowerMsg.includes('no') && lowerMsg.length < 50); // Any "No" in context
        ...
        if (isSimpleConfirmation) {
            ...
            try {
                ...
                const confirmedAppts = appts.filter(a => a.status === 'completed');
        ...
        // Helper: Verifica los próximos días y devuelve los que realmente tienen disponibilidad
        const getNextAvailableDaysInfo = async (startFromDate, maxDays = 10) => {
            const results = [];
            const nowForNext = new Date(new Date().toLocaleString('en-US', { timeZone: 'America/Guayaquil' }));
            const todayStrNext = `${nowForNext.getFullYear()}-${(nowForNext.getMonth() + 1).toString().padStart(2, '0')}-${nowForNext.getDate().toString().padStart(2, '0')}`;
            
            const startDate = new Date(startFromDate + 'T12:00:00-05:00');
            ...
            for (let i = 0; i < maxDays; i++) {
                ...
                if (dateStr <= todayStrNext) continue; // Skip past/today
                
                const checkDay = checkDate.getUTCDay();
                if (checkDay === 0) continue; // Skip Sunday
                
                // Check if blocked
                const [blockedRows] = await db.execute("SELECT reason FROM blocked_dates WHERE DATE_FORMAT(blocked_date, '%Y-%m-%d') = ?", [dateStr]);
                if (blockedRows.length > 0) continue; // Skip blocked day
                
                // Check availability
                try {
                    const daySlots = await getAvailableSlots(dateStr, 45, cleanPhone);
                    if (daySlots.length > 0) {
                        const dayNames = ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado'];
                        const displaySlots = daySlots.filter(s => s !== "14:00" && s !== "16:00" && s !== "08:00");
                        results.push(`${dayNames[checkDay]} ${dateStr} (${displaySlots.join(', ')})`);
                        if (results.length >= 3) break; // Max 3 alternatives
                    }
                } catch (e) {
        ...
        const isExplicitBooking = normalizedMsg.includes('cita') || normalizedMsg.includes('agendar') || normalizedMsg.includes('horario') || normalizedMsg.includes('disponibilidad') || normalizedMsg.includes('consulta') || normalizedMsg.includes('revision');
        ...
        if (isExplicitBooking || isCurrentlyInBookingFlow || resolvedDate) {
            ...
            const duration = 45;
            const isOrtho = normalizedMsg.includes('bracket') || normalizedMsg.includes('frenillo') || normalizedMsg.includes('ortodoncia');
            const isPediatric = normalizedMsg.includes('nino') || normalizedMsg.includes('hijo') || normalizedMsg.includes('infantil');
            const isProsthesis = normalizedMsg.includes('protesis') || normalizedMsg.includes('placa') || normalizedMsg.includes('diente postizo') || normalizedMsg.includes('puente');
            const isSurgery = normalizedMsg.includes('cirugia') || normalizedMsg.includes('tercer molar') || normalizedMsg.includes('muela del juicio') || normalizedMsg.includes('extraccion');
                    ...
                    if (targetDate < todayStrLogic) {
                        ...
                        if (targetDate > maxStrLogic) {
                            ...
                            if (isBlockedRows.length > 0) {
                                ...
                                const nextBlockedInfo = await getNextAvailableDaysInfo(targetDate);
                                let altBlocked = 'alternativas (días cercanos con horarios)';
                                if (nextBlockedInfo.length > 0) {
                                    altBlocked = 'ALTERNATIVAS REALES disponibles: ' + nextBlockedInfo.join('; ');
                                } else {
                                    altBlocked = 'No se encontraron días cercanos con disponibilidad. Pídele al usuario que elija otra fecha.';
                                }
                                availabilityContext = `[ESTADO: DÍA NO DISPONIBLE. El ${targetDate} no tiene disponibilidad. El usuario pregunta por este día específicamente. IMPORTANTE: Dile directamente que "No tenemos disponibilidad para ese día" o "Ese día no estamos atendiendo". NO menciones horarios, NO pidas datos, NO ofrezcas la hora específica que pidió. Solo indica que no hay disponibilidad y ofrece ${altBlocked}.]`;
                                slots = [];
                            } else {
                                ...
                                if (dateTruth.includes('Cerrado')) {
                                    const nextClosedInfo = await getNextAvailableDaysInfo(targetDate);
                                    let altClosed = 'otro día';
                                    if (nextClosedInfo.length > 0) {
                                        altClosed = 'ALTERNATIVAS REALES disponibles: ' + nextClosedInfo.join('; ');
                                    } else {
                                        altClosed = 'No se encontraron días cercanos con disponibilidad. Pídele al usuario que elija otra fecha.';
                                    }
                                    availabilityContext = `[ESTADO CRÍTICO: CERRADO para ${targetDate}. ${dateTruth}. NO HAREMOS CITAS. Dile al usuario que ese día estamos cerrados y ofrece ${altClosed}.]`;
                                } else if (slots.length === 0) {
                                    const nextNoSlotsInfo = await getNextAvailableDaysInfo(targetDate);
                                    let altNoSlots = 'días alternativos cercanos';
                                    if (nextNoSlotsInfo.length > 0) {
                                        altNoSlots = 'ALTERNATIVAS REALES disponibles: ' + nextNoSlotsInfo.join('; ');
                                    } else {
                                        altNoSlots = 'No se encontraron días cercanos con disponibilidad. Pídele al usuario que elija otra fecha.';
                                    }
                                    availabilityContext = `[ESTADO: SIN HORARIOS DISPONIBLES para ${targetDate}. El día está abierto pero todos los turnos están ocupados. Indica que no hay cupos y ofrece ${altNoSlots}.]`;
                                } else if (slots.length > 0) {
        ...
        if (bestMatch || proposedTime) {
            ...
            if (!isWithinBusinessHours && targetDate) {
                ...
                const isChild = lowerMsg.includes('niño') || lowerMsg.includes('hijo') || lowerMsg.includes('hija') || lowerMsg.includes('pequeño');
                const dataReq = isChild ? "(Nombre del niño y Nombre del Representante)" : "(Nombre Completo)";
                availabilityContext += `\n[SLOT VERIFICADO: ${finalTime} está DISPONIBLE para el ${targetDate}. AHORA solicite los datos faltantes: ${dataReq}. Use USTED.]`;
            } else {
        ...
        const patientContext = `PACIENTE: ${session.name || 'Desconocido'}. Estado: ${session.current_flow}`;
        ...
        const systemPrompt = `
            ...
            REGLA DE ORO PROHIBITIVA: Si el sistema indica [ESTADO: DÍA NO DISPONIBLE], [ESTADO CRÍTICO: CERRADO] o [ESTADO: SIN HORARIOS DISPONIBLES], está ESTRICTAMENTE PROHIBIDO decir "tenemos disponibilidad de 09:00 a 12:30..." ni citar los horarios generales de atención. DEBES decir claramente que para ese día NO hay atención/disponibilidad y ofrecer los días alternativos indicados.
        ...
        const userConfirmingSi = messageText.toLowerCase().trim() === 'si' || 
                                 ...
                                 messageText.toLowerCase().trim() === 'si, confirmo';
        const botAskingForData = botResponse.includes('nombre') || botResponse.includes('datos');
        ...
        // Fallback: If no metadata but bot said "confirmed"
            if (!metadata && (
                ...
                if (session.target_date && session.target_time && session.name) {
                    metadata = {
                        action: "create_appointment",
                        name: session.name || "Paciente",
                        cedula: session.cedula || null,
                    ...
                    console.log(`[Logic] Session fallback FAILED. Missing data: Name:${session.name}, Date:${session.target_date}, Time:${session.target_time}`);
                }
            }
        ...
        if (metadata && metadata.action === 'create_appointment' && !bookingExecuted) {
            ...
            console.log(`[DEBUG-FLOW] metadata.name: "${metadata.name}", session.name: "${session.name}"`);

            // NAME IS THE ONLY MANDATORY FIELD FOR BOOKING
            if (!metadata.name && !session.name) {
                console.log(`[Logic] Name is missing. Asking for patient name.`);
                const isChild = lowerMsg.includes('niño') || lowerMsg.includes('hijo') || lowerMsg.includes('hija');
            ...
            // Only proceed to booking if we have the name
            if (!bookingExecuted) {
