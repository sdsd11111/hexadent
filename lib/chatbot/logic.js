    ...
    if (!messageText) return;
        ...
        if (metadata && metadata.action === 'create_appointment' && !bookingExecuted) {
            ...
            if (!metadata.name) {
                ...
                // Name exists but no valid ID → tell LLM to ask for CONFIRMATION, not more data
                console.log(`[Logic] Name present but no valid ID. Forcing confirmation flow.`);
                availabilityContext = `[SISTEMA: El paciente YA proporcionó su nombre (${metadata.name}). NO pidas más datos. Pide confirmación directa así: "¿Confirma su cita para el ${session.target_date} a las ${session.target_time}?" - No menciones que falta cédula o edad.]`;
                // Flag session so fallback knows to proceed
                session._skipDataRequest = true;
                bookingExecuted = true; // STOP here, let LLM ask for confirmation
            } else {
