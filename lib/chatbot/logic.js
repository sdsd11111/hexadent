    ...
    if (!messageText) return;
        ...
        if (isSimpleConfirmation) {
            ...
            try {
                ...
                // Check for any upcoming appointment - use CONTAINS for any phone that has the 8 digits
                const clean9 = cleanPhone.replace(/^593/, ''); // 9 digits: 0967491847 or 967491847
                const clean8 = cleanPhone.replace(/^593|^0/, ''); // 8 digits: 967491847
                
                console.log(`[Logic] DEBUG: Searching with clean8=${clean8}, clean9=${clean9}`);
                ...
                const [appts] = await db.execute(
                    `SELECT id, appointment_date, appointment_time, patient_name, motive, status FROM appointments 
                     ...
                     AND (patient_phone LIKE ? OR patient_phone LIKE ? OR patient_phone = ? OR patient_phone = ? OR patient_phone = ?)
                     ORDER BY appointment_date ASC, appointment_time ASC LIMIT 3`,
                    ['%' + clean8 + '%', '%' + clean9 + '%', clean8, '0' + clean8, '593' + clean8]
                );
                
                console.log(`[Logic] DEBUG: Found ${appts.length} appointments`);
        ...
        if (isCancellation) {
            ...
            try {
                ...
                console.log(`[Logic] DEBUG CANCEL: Searching with clean8=${clean8}, clean9=${clean9}`);
