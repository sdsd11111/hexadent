...
export async function processChatbotMessage(phoneNumber, messageText, clinicNumber = null) {
    ...
    try {
        ...
        if (lowerMsg.includes('cita') || lowerMsg.includes('disponibilidad') || lowerMsg.includes('cuándo') || lowerMsg.includes('horario') || lowerMsg.includes('cuanto') || lowerMsg.includes('hay') || lowerMsg.includes('se puede') || lowerMsg.includes('tienes espacio') || lowerMsg.includes('puedo ir')) {
            ...
            ⚠️ REGLA CRÍTICA PARA EL ASISTENTE: El ${targetDate} es ${dayNameEcu.toUpperCase()}. Si vas a mencionar el nombre del día, di ESTRICTAMENTE "${dayNameEcu}". NUNCA lo llames de otra forma.${timeWarning}`;
...
${getReferenceCalendar()}
        ...
        5. Si hay una advertencia (timeWarning), NO digas "Perfecto". Di: "Lo siento, ese horario no es válido..." y explica por qué.
        6. FECHAS: Rechaza fechas pasadas o inexistentes (Feb 29 en 2026/2027).
        7. Al confirmar, menciona la fecha EXACTA (día y número) del availabilityContext.
