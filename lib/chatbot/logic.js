    ...
    if (!messageText) return;
        ...
        if (bestMatch || proposedTime) {
            ...
            if (isTargetSaturday) {
                ...
                // Lunes a Viernes: 09:00 a 13:00 y 15:00 a 18:00 + Especial 14:00
                const timeInMinutes = finalH * 60 + finalM;
            ...
            if (!isWithinBusinessHours && targetDate) {
                let hoursMessage = isTargetSaturday 
                    ...
                    : "de lunes a viernes atendemos de 09:00 a 13:00 y de 15:00 a 18:00 (último turno 17:30)";
        ...
        const systemPrompt = `
            ...
            REGLA DE FORMATO DE HORARIOS TARDE: Para horarios de la tarde (15:00-18:00), está PROHIBIDO listar cada slot individual si hay muchos seguidos. En su lugar, menciona el rango disponible (ej: "a partir de las 15:45"). Si solo hay UNO o POCOS slots disponibles, indícalos directamente (ej: "Solo tenemos disponible a las 16:30"). NUNCA digas que no hay disponibilidad si ves al menos un horario en la lista de arriba.
            ...
            - Lunes-Viernes: 09:00 a 13:00 y 15:00 a 18:00. EL ÚLTIMO TURNO POSIBLE ES A LAS 17:30.
            ...
            CIERRE DE JORNADA: Muestra los horarios hasta el final (hasta las 17:30 de lunes a viernes, o hasta las 14:30 los sábados). Todos los turnos son de 45 minutos.
        ...
        // Check if session has name from previous messages and we don't have full data yet
        const nameFromSession = session.name;
        const hasPartialData = nameFromSession && !session.cedula && !idMatch;
        const wasAskingForData = botResponse.includes('cédula') || botResponse.includes('edad') || botResponse.includes('datos');
        
        let finalResponseText;
        if (hasPartialData && wasAskingForData && session.target_date && session.target_time) {
            console.log(`[Logic] OVERRIDE: User gave only name. Forcing confirmation instead of asking for more data.`);
            ...
            const firstName = nameFromSession.split(' ')[0];
