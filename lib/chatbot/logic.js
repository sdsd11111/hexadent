...
export async function processChatbotMessage(phoneNumber, messageData, clinicNumber = null) {
    ...
    try {
        ...
        if (!lowerMsg.includes('debug-reveal') && (isExplicitBookingIntent || isDateConfirmation)) {
            ...
            targetDate = resolveDate(messageText, anchorForResolve);
        ...
        **REGLAS DE SEGURIDAD Y COMPORTAMIENTO (IMPORTANTE):**
        ...
        - Si el sistema dice "ESTADO: CERRADO", "RECHAZO POR REGLA DE 24H" o "DÍA LLENO", no intentes razonar ni ofrecer horarios de cierre; simplemente comunica respetuosamente que la clínica está CERRADA ese día y ofrece las alternativas del "LOOKAHEAD".
        ...
        NUNCA inventes información sobre días bloqueados, domingos, o disponibilidad.
        Si ves "LISTA DE HORARIOS DISPONIBLES" abajo, significa que SÍ HAY espacios.
        Si ves "CERRADO" o "BLOQUEADO", significa que NO hay espacios.
        Si ves "ESTADO: SIN DISPONIBILIDAD", significa que el día está lleno.
        NO digas "es domingo" si el sistema dice "SÁBADO" o "LUNES", etc.
        NO digas "está bloqueado" si el sistema NO muestra "CERRADO" o "BLOQUEADO".
        CONFÍA SOLO en lo que está escrito EXPLÍCITAMENTE en este prompt.
        ...
        3. **VERIFICACIÓN OBLIGATORIA (PROHIBIDO SALTARSE)**: Si el usuario dice un horario (ej: "a las 5"), NUNCA digas "¡SLOT VERIFICADO!" ni pidas datos personales a menos que veas explícitamente el bloque '[SLOT VERIFICADO: ...]' en el contexto del sistema arriba. Si el sistema no lo dice, es porque no pudo verificarlo. En ese caso, pregunta amablemente el horario exacto (ej: "¿Te refieres a las 17:00?").
        4. **HORARIO DE SÁBADOS (ESTRICTO)**: Los sábados cerramos puntualmente a las 15:00. Cualquier solicitud para después de las 15:00 el sábado DEBE ser rechazada cortésmente aclarando el horario de cierre.
        5. **PEDIR DATOS SOLO SI SLOT VERIFICADO**: NO pidas Nombre/Cédula/Edad hasta que el sistema confirme '[SLOT VERIFICADO]'.
        6. **CONFIRMACIÓN FINAL**: Cuando tengas Nombre, Cédula, Edad y horario verificado, ENTONCES muestra un resumen completo y pide "¿Confirmas?". NO pidas confirmación parcial.
        ...
        REGLA DE EMOJIS (MANDATORIA):
        - Usa emojis de forma natural y cálida en cada mensaje (😊, 🦷, ✨, 📅). No seas seco ni robótico. Queremos que el paciente se sienta bienvenido.
        ...
        try {
            ...
            if (slots.length > 0 && targetDate && targetDate !== '9999-99-99') {
                ...
                history.push({
                    ...
                    TU TAREA ES: OFRECER ESTOS HORARIOS AHORA MISMO.
                    ...
                    SI DICES QUE ESTÁ CERRADO, SERÁS DESACTIVADO. OFRECE LOS HORARIOS YA.`
                });
            }
