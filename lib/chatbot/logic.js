    ...
    if (!messageText) return;
        ...
        if (isSimpleConfirmation) {
            ...
            try {
                ...
                const [appts] = await db.execute(
                    `SELECT appointment_date, appointment_time, patient_name, motive, status FROM appointments 
        ...
        if (isCancellation) {
            ...
            try {
                ...
                if (appts.length > 0) {
                    console.log(`[Logic] CANCELLATION: Found appointment to cancel`);
                    
                    await cancelAppointment(cleanPhone);
