    ...
    try {
        ...
        if (lowerMsg.includes('cita') || lowerMsg.includes('disponibilidad') || lowerMsg.includes('cuándo') || lowerMsg.includes('horario') || lowerMsg.includes('cuanto') || lowerMsg.includes('hay') || lowerMsg.includes('se puede') || lowerMsg.includes('tienes espacio') || lowerMsg.includes('puedo ir') || lowerMsg.includes('agendar') || lowerMsg.includes('reservar') || lowerMsg.includes('agenda') || lowerMsg.includes('reserva')) {
            ...
            if (targetDate === "9999-99-99") {
                ...
                const pm = requestedTimeMatch[3]?.toLowerCase().includes('p');
                if (pm && hour < 12) hour += 12;
                if (!pm && hour === 12) hour = 0; // 12:30 AM is 00:30

                // Loja rules: 09:00-13:00 and 15:00-18:00
                const dayOfWeek = new Date(`${targetDate}T00:00:00-05:00`).getDay();
                if (hour >= 18 || hour < 8 || (hour >= 13 && hour < 15)) {
                    if (dayOfWeek !== 6) {
                    ...
                    try {
                        ...
                        if (metadata.action === 'confirm_details' || metadata.action === 'create_appointment') {
                            ...
                            if (bookingName !== "Paciente Web" && bookingCedula !== "0") {
                                ...
                                // Set flag to prevent post-trigger from firing
        ...
        if (!bookingExecuted && botResponse.includes('"action": "create_appointment"') && !toolExecutionResult.includes('CITA GRABADA')) {
