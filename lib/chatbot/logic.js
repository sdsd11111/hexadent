    ...
    try {
        ...
        if (bestMatch || proposedTime) {
            ...
            if (slots.includes(finalTime) || (isTargetSaturday && finalTime === "08:30")) {
                ...
                availabilityContext += `\n[SLOT VERIFICADO: ${finalTime} está DISPONIBLE para el ${targetDate}. AHORA solicite los datos faltantes de forma amable (Nombre, Cédula, Edad). No pida lo que ya tenga en el contexto. Use USTED.]`;
            } else {
        ...
        if (!metadata && (
            ...
            if (session.target_date && session.target_time && (session.cedula || idMatch)) {
                metadata = {
                    action: "create_appointment",
                    name: session.name || "Paciente",
                    cedula: session.cedula || (idMatch ? idMatch[1] : null),
                    age: session.age,
