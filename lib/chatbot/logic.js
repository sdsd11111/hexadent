    ...
    try {
            ...
            if (lowText.includes('pasado mañana')) date.setDate(date.getDate() + 2);
                ...
                for (const [name, val] of Object.entries(dayMap)) {
                    if (lowText.includes(name)) {
                        ...
                        // AUTO-SHIFT: If result is today and time has passed, move to next week
                        const nowEcu = new Date(now.getTime() - (5 * 60 * 60 * 1000));
                        const dateStr = nowEcu.toISOString().split('T')[0];
                        ...
                        if (dateStr === resultStr) {
                            ...
                            // If it's already past 18:00 (closing), shift today to next week
                            if (currentTime >= 18) {
        ...
        if (lowerMsg.includes('cita') || lowerMsg.includes('disponibilidad') || lowerMsg.includes('cuándo') || lowerMsg.includes('horario') || lowerMsg.includes('cuanto') || lowerMsg.includes('hay') || lowerMsg.includes('se puede') || lowerMsg.includes('tienes espacio') || lowerMsg.includes('puedo ir')) {
            ...
            if (targetDate === "9999-99-99") {
                timeWarning = "\n⚠️ REGLA DE HARDENING: Fecha inexistente (ej: 32 de febrero). RECHAZA AMABLEMENTE Y EXPLICA QUE NO EXISTE.";
                ...
                if (hour >= 15 || (hour === 8 && parseInt(requestedTimeMatch[2] || "0") < 30)) {
                    timeWarning = `\n⚠️ REGLA DE HARDENING: El horario ${hour}:${requestedTimeMatch[2] || "00"} del Sábado está fuera de rango (08:30-15:00). RECHAZA AMABLEMENTE Y EXPLICA QUE CERRAMOS A LAS 15:00.`;
                }
                ...
                if (hour >= 18 || hour < 9 || (hour >= 13 && hour < 15)) {
                    timeWarning = `\n⚠️ REGLA DE HARDENING: El horario ${hour}:${requestedTimeMatch[2] || "00"} de L-V está fuera de rango (9:00-13:00 / 15:00-18:00). RECHAZA AMABLEMENTE.`;
                }
            } else if (targetDate === dateStr && requestedTimeMatch) {
                // If today, check if requested time is in the past
                let reqHour = parseInt(requestedTimeMatch[1]);
                const pm = requestedTimeMatch[3]?.toLowerCase().includes('p');
                if (pm && reqHour < 12) reqHour += 12;
                if (!pm && reqHour === 12) reqHour = 0;

                const nowEcu = new Date(now.getTime() - (5 * 60 * 60 * 1000));
                const currentHour = nowEcu.getUTCHours();
                const currentMin = nowEcu.getUTCMinutes();
                const currentTotal = currentHour + (currentMin / 60);
                const reqTotal = reqHour + (parseInt(requestedTimeMatch[2] || "0") / 60);

                if (reqTotal < currentTotal) {
                    timeWarning = `\n⚠️ REGLA DE HARDENING: El usuario pide hoy a las ${reqHour}:${requestedTimeMatch[2] || "00"}, pero YA SON LAS ${currentHour}:${currentMin}. NO PUEDES AGENDAR AL PASADO. Sugiere mañana o el próximo disponible.`;
                }
            } else if (targetDate < dateStr) {
                timeWarning = "\n⚠️ REGLA DE HARDENING: El usuario pide una fecha PASADA. RECHAZA AMABLEMENTE (Estamos en el presente).";
            }
            ...
            availabilityContext = `LISTA DE HORARIOS DISPONIBLES PARA EL ${dayNameEcu} ${targetDate} (${messageText}) - Duración estimada: ${duration} min: ${slots.length > 0 ? slots.join(', ') : 'No hay turnos disponibles para este día.'}
            ⚠️ REGLA DE ORO DE HORARIOS:
            - ${sessionInfo}
            - NUNCA menciones las ${morningLimit}:00 o ${afternoonLimit}:00 como opción de inicio.
            - Si el horario que pide el usuario (ej: 12:30 o 17:30) está en la LISTA de arriba, ACÉPTALO. No digas que cerramos si todavía hay tiempo para los ${duration} min de tratamiento.${timeWarning}`;
...
${getReferenceCalendar()}
        ...
        4. Si el usuario quiere dos tratamientos (ej: caries y brackets), puedes agendar ambas seguidas o en una sola cita más larga. NO digas "No se puede agendar seguido".
        5. ÚLTIMA CITA: La clínica CIERRA a las 18:00 (L-V) o 15:00 (Sáb). Es imposible empezar una cita a la hora de cierre. La última cita debe empezar antes de la hora de cierre (ej: 17:40 para algo de 20 min). NUNCA digas que la última cita es a las 18:00 o 15:00.
        6. Si hay una advertencia (timeWarning), NO digas "Perfecto". Di: "Lo siento, ese horario no es válido..." y explica por qué.
        7. FECHAS: Rechaza fechas pasadas o inexistentes (Feb 29 en 2026/2027).
...
async function logActivity(phone, userMsg, botResp) {
    // This function is now unused as logging is handled directly in handleIncomingMessage
    // Keeping it for now in case it's referenced elsewhere or for future use.
    try {
        await db.execute(
