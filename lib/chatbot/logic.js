...
export async function processChatbotMessage(phoneNumber, messageData, clinicNumber = null) {
    ...
    if (!messageText) return;
        ...
        console.log(`[Logic] Processing message: "${messageText}" (lower: "${lowerMsg}")`);
        ...
        // Si es confirmación, forzar que NO sea primer mensaje (ignorar regla de saludo)
        if (isSimpleConfirmation) {
            console.log(`[Logic] Confirmation detected: "${messageText}"`);
        }
        ...
        if (isSimpleConfirmation) {
            ...
            try {
                // Generate all possible phone formats for matching - use CONTAINS approach
                const clean8 = cleanPhone.replace(/^593|^0/, ''); // 8 digits: 967491847
                
                console.log(`[Logic] Searching with 8-digit: ${clean8}`);
                
                // Check for any upcoming appointment - match by containing the 8 digits
                const [appts] = await db.execute(
                    `SELECT id, appointment_date, appointment_time, patient_name, motive, status FROM appointments 
                     ...
                     AND (patient_phone LIKE ? OR patient_phone LIKE ? OR patient_phone LIKE ? OR patient_phone = ? OR patient_phone = ?)
                     ORDER BY appointment_date DESC, appointment_time DESC LIMIT 3`,
                    ['%' + clean8, '%' + clean8 + '%', clean8, '0' + clean8, '593' + clean8]
                );
                
                if (appts.length > 0) {
                    console.log(`[Logic] CONFIRMATION SUCCESS! Found ${appts.length} appointment(s)`);
                    
                    const formatDate = (d) => {
                        if (!d) return 'HOY';
                        const dateStr = String(d).split('T')[0];
                        if (!dateStr || dateStr === 'undefined') return 'HOY';
                        const dateObj = new Date(dateStr + 'T12:00:00-05:00');
                        if (isNaN(dateObj.getTime())) return 'HOY';
                    ...
                    let responseMsg = "✅ *¡Su cita está confirmada!*\n\n";
                    for (const appt of appts) {
                        // Clean motive - remove SPECIAL tags
        ...
        if (isCancellation) {
            ...
            try {
                const clean8 = cleanPhone.replace(/^593|^0/, '');
                
                const [appts] = await db.execute(
                    `SELECT id, appointment_date, appointment_time FROM appointments 
                     WHERE status = "scheduled" AND appointment_date >= CURDATE() 
                     AND (patient_phone LIKE ? OR patient_phone LIKE ? OR patient_phone = ? OR patient_phone = ?)
                     LIMIT 1`,
                    ['%' + clean8, clean8, '0' + clean8, '593' + clean8]
                );
                
                if (appts.length > 0) {
                    console.log(`[Logic] CANCELLATION: Found appointment to cancel, ID: ${appts[0].id}`);
        ...
        // IMPORTANTE: Si es confirmación, NO aplicar regla de saludo
        const isFirstMessage = history.length === 0 && !isSimpleConfirmation;
