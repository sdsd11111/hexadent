...
export async function processChatbotMessage(phoneNumber, messageData, clinicNumber = null) {
    ...
    try {
        ...
        // Si termina con ? pero NO tiene ¿, reemplazar con emoji alternando aleatoriamente
        if (!finalResponseText.includes('¿') && finalResponseText.trim().endsWith('?')) {
        ...
        finalResponseText = finalResponseText.replace(/\?\s+(?=[A-ZÁÉÍÓÚ¿])/g, () => Math.random() > 0.5 ? ' 🦷 ' : ' 😊 ');
        
        // 6. Asegurar espaciado correcto de emojis alrededor de puntuación
        // El emoji debe ir DESPUÉS del ? cuando es al final de pregunta, no antes
        // Eliminar las reglas que movían el emoji antes del ? y causaban " ? ?"
