...
export async function processChatbotMessage(phoneNumber, messageText, clinicNumber = null) {
    ...
    try {
        ...
        // --- RADICAL DEBUG: IMMEDIATE INTERCEPTION ---
        // Bypass EVERYTHING (DB, OpenAI, Calendar) if this keyword is found
        if (messageText && messageText.toLowerCase().includes('debug-reveal')) {
            const calId = process.env.GOOGLE_CALENDAR_ID;
            const saKey = process.env.GOOGLE_SERVICE_ACCOUNT_KEY;
            console.log('[Chatbot Logic] RADICAL DEBUG TRIGGERED');
            return {
                success: true,
                response: `[RADICAL DEBUG]\nENV CHECK:\n- CALENDAR_ID: ${calId ? (calId.substring(0, 5) + '...') : 'MISSING'}\n- SA_KEY: ${saKey ? 'PRESENT' : 'MISSING'}\n\nSystem is alive. If you see this, Webhook is OK.`
            };
        }
