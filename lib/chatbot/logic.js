...
export async function processChatbotMessage(phoneNumber, messageText, clinicNumber = null) {
    ...
    try {
        // --- 0. CHECK IGNORED NUMBERS ---
        const [ignored] = await db.execute('SELECT 1 FROM ignored_numbers WHERE phone = ?', [phoneNumber.replace(/\D/g, '')]);
        if (ignored.length > 0) {
            console.log(`[Chatbot Logic] Skipping ignored number: ${phoneNumber}`);
        ...
        // Send back via WhatsApp (Exclusive Evolution API)
