...
export async function processChatbotMessage(phoneNumber, messageData, clinicNumber = null) {
    ...
    // CRITICAL: Define 'now' for all date calculations (Ecuador Time)
    ...
    try {
        ...
        // --- NUCLEAR SAFETY VALVE: If session DATE is in MARCH while we are in FEBRUARY, kill it ---
        if (session.target_date && session.target_date.includes('-03-') && dateStr.includes('-02-')) {
            console.warn(`[Logic] Safety Valve triggered: Nullifying legacy March date ${session.target_date} for ${cleanPhone}`);
            session.target_date = null;
            session.target_time = null;
            session.current_flow = 'Sesión reiniciada por error de fecha';
        }
        ...
        // --- 2. CURRENT TIME ALREADY DEFINED AT TOP ---
        const earliestPossible = new Date(now.getTime() + (24 * 60 * 60 * 1000));

        // --- 2.5 IMPROVED DATA EXTRACTION (HEURISTICS) ---
        // Extract Age: look for "XX años" or "tengo XX"
        ...
        if (ageMatch && !session.age) {
            ...
            console.log(`[Logic] Heuristic Age extracted: ${session.age}`);
        }
        // Extract Cedula: look for 10 digits
        const idMatch = messageText.match(/\b(\d{10})\b/);
        if (idMatch && !session.cedula) {
            session.cedula = idMatch[1];
            console.log(`[Logic] Heuristic Cedula extracted: ${session.cedula}`);

            // Extract Name Heuristic: If we have a cedula, everything before it in the same message is likely the name
            const textBeforeId = messageText.split(idMatch[1])[0].trim();
            if (textBeforeId.length > 3 && textBeforeId.split(' ').length >= 2 && !session.name) {
                // Clean up common prefix words
                const cleanName = textBeforeId.replace(/soy|me llamo|mi nombre es|un gusto/gi, '').trim();
                session.name = cleanName;
                console.log(`[Logic] Heuristic Name extracted: ${session.name}`);
            }
        }
        ...
        if (!lowerMsg.includes('debug-reveal') && (isExplicitBookingIntent || isDateConfirmation || isCurrentlyInBookingFlow)) {
            ...
            // SOBERANÍA DE FECHA: Si ya hay una fecha en el futuro lejano (no este mes), 
            // no permitimos que un número suelto (fuzzy) la cambie a menos que mencionen el mes.
            let finalResolved = resolved;
            if (resolved && session?.target_date && resolved !== session.target_date) {
                const firmRegex = /enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre|hoy|mañana|pasado|lunes|martes|miércoles|miercoles|jueves|viernes|sábado|sabado|domingo/i;
                const currentMonth = new Date().toISOString().substring(0, 7);
                const targetMonth = (typeof session.target_date === 'string') ? session.target_date.substring(0, 7) : null;

                if (targetMonth && targetMonth !== currentMonth && !firmRegex.test(messageText)) {
                    console.log(`[Logic] Protected far date ${session.target_date} from fuzzy update ${resolved} via: "${messageText}"`);
                    finalResolved = null;
                }
            }

            targetDate = finalResolved || session?.target_date || dateStr;
            // Defensive Normalization: Ensure targetDate is a string (YYYY-MM-DD)
            ...
            if (finalResolved && finalResolved !== session?.target_date) {
                await saveSession(cleanPhone, { target_date: finalResolved });
            }
            ...
            const hasMotiveKeyword = isOrthoTreatment || isOrthoEval || isPediatric || isGeneral;
            const hasVerbMotive = lowerMsg.includes('cita para') || lowerMsg.includes('consulta de') || lowerMsg.includes('tengo caries') || lowerMsg.includes('quiero limpieza');
            const hasMotive = hasMotiveKeyword || hasVerbMotive;
            ...
            if (hasActiveMotive) {
                ...
                if (isBlocked.length > 0) {
                    ...
                    const allSlotsText = slots.join(', ');
                    availabilityContext = `${dateTruthBlock}\n⚠️ HORARIOS REALES DISPONIBLES PARA EL ${dayNameEcu.toUpperCase()} ${targetDate} (Duración: ${duration}min):\n${allSlotsText}\n\nREGLA: Debes mostrar esta lista COMPLETA al usuario. Los horarios que NO están en esta lista están OCUPADOS o FUERA DE JORNADA.`;
            ...
            // CRITICAL: If slots is empty but we have a date/motive, ensure AI doesn't hallucinate
            if (hasActiveMotive && targetDate && targetDate !== '9999-99-99' && slots.length === 0) {
                const nextDay = new Date(new Date(targetDate + 'T12:00:00-05:00').getTime() + 24 * 60 * 60 * 1000).toISOString().split('T')[0];
                availabilityContext += `\n⚠️ AVISO LEGAL DE DISPONIBILIDAD: EL DÍA ${targetDate} ESTÁ BLOQUEADO EN LA BASE DE DATOS (REGLA 24H O CIERRE MANUAL). 
                NO TIENES PERMISO PARA SUGERIR HORAS. 
                DEBES EXPLICAR QUE NO HAY CUPOS Y PROPONER EL SIGUIENTE DÍA DISPONIBLE (EJ: EL SÁBADO O LUNES).`;

                history.push({
                    role: "system",
                    content: `🔴 ALERTA DE SISTEMA: El motor de reservas confirma que NO HAY DISPONIBILIDAD para el ${targetDate}. 
                    ESTÁ PROHIBIDO dar horarios inventados. 
                    TU MISIÓN: Di que no hay cupos por política de anticipación o bloqueo, y sugiere proactivamente el siguiente día laborable (Sábado o Lunes).`
                });
            }
            ...
            // --- 8. DETECT TIME SELECTION ---
            // Improved regex: handles "8:30", "8h30", "8 y 30", "8 30", "8 am"
            const timePatterns = /(?:a\s+las\s+)?(\d{1,2})(?:\s*[:h\s.\-y]\s*(\d{2}))?(\s*[ap]m)?/gi;
            let timeMatch = null;
            let proposedTime = null;
            let bestMatch = null;

            // Search through all numeric patterns in the message to find one that matches an available slot
            let m;
            while ((m = timePatterns.exec(lowerMsg)) !== null) {
                ...
                if (!ampm && hh > 0 && hh <= 7) hh += 12; // Heuristic for afternoon hours

                const testTime = `${hh.toString().padStart(2, '0')}:${mm}`;

                // If this match exactly exists in our slots, IT IS THE ONE.
                if (slots.includes(testTime)) {
                    bestMatch = testTime;
                    break;
                }

                // Keep the first one as fallback if no perfect match found
                if (!proposedTime) proposedTime = testTime;
            }

            if (bestMatch || proposedTime) {
                const finalTime = bestMatch || proposedTime;
                const isSlotAvailable = slots.includes(finalTime);

                if (isSlotAvailable) {
                    session.target_time = finalTime;
                    await lockSlot(cleanPhone, session.target_date, finalTime);
                    availabilityContext = `[SLOT VERIFICADO: ${finalTime} está DISPONIBLE para el ${session.target_date}. Ahora pide los datos personales (Nombre, Cédula, Edad) si aún no los tienes. NO pidas confirmación todavía.]`;
                } else {
                    availabilityContext = `[SLOT NO DISPONIBLE: ${finalTime} ya NO está libre para el ${session.target_date}. Ofrece otros horarios de la lista. NO pidas datos personales.]`;
                    session.target_time = null;
                }
            }
        ...
        // Generate a literal reference calendar for the next 14 days (reduced to avoid AI confusion)
        const getReferenceCalendar = () => {
            const lines = [];
            let d = new Date(now.getTime());
            const formatter = new Intl.DateTimeFormat('es-EC', { weekday: 'long', timeZone: 'America/Guayaquil' });
            lines.push(`CALENDARIO DE REFERENCIA (Próximos 14 días):`);
            for (let i = 0; i < 14; i++) {
                ...
                lines.push(`➡️ ${dName.toUpperCase()} ${dStr}${closureInfo}${i === 0 ? ' (HOY - FEBRERO)' : ''}`);
                d.setDate(d.getDate() + 1);
            }
            lines.push(`\nREGLA: Confía únicamente en este calendario para saber qué día es mañana o pasado.`);
            return lines.join('\n');
        };
        ...
        **IDENTIDADDEL PACIENTE:**
        ...
        1. **FECHA ACTUAL**: HOY ES ${dayEcu} ${dateStr}. ESTAMOS EN **FEBRERO**. 
        2. **¡PROHIBIDO MARZO!**: NO menciones el mes de MARZO bajo ninguna circunstancia a menos que el usuario lo pida. El bot tiene un bug donde confunde febrero con marzo. HOY ES FEBRERO.
        3. **FECHAS LEJANAS**: Si un usuario pide una fecha lejana, felicítalo.
        3. **SÁBADOS VS SEMANA**: 
           - Entre semana (M-V) hay almuerzo de 13:00 a 15:00.
           - LOS SÁBADOS NO HAY ALMUERZO. SE ATIENDE CORRIDO DE 08:30 A 15:00.
           - Si un usuario pide las 14:00 el sábado y el sistema lo permite, ¡AGÉNDALO! Está abierto.
        4. **CALENDARIO**:
        ...
        - Si ves un horario en la lista "HORARIOS REALES DISPONIBLES", ese horario ES 100% VÁLIDO Y AGENDABLE. 
        - NUNCA digas que no está disponible si aparece en la lista. 
        - Si el usuario pide un horario que TU ACABAS DE MOSTRAR EN TU MENSAJE ANTERIOR, acepta ese horario INMEDIATAMENTE. No vuelvas a verificar.
        - Si el usuario lo pide y está en la lista (como las 08:30 o 14:30), procede directamente. No pongas excusas.
        - TU CEREBRO ESTÁ CONECTADO A LA MISMA BASE DE DATOS QUE EL CALENDARIO DE LA WEB.
        - Lo que ves en 'HORARIOS REALES DISPONIBLES' es la VERDAD ABSOLUTA del servidor.
        - Ignora cualquier otra "lógica" de horarios que tú creas tener si contradice la lista.
        - La lista es la ÚNICA verdad. Si no está en la lista, no existe.
        
        ⚠️ REGLA DE EXHIBICIÓN DE HORARIOS (HORIZONTAL SIEMPRE):
        - DEBES mostrar la lista de horarios EXACTAMENTE como se te entrega.
        - USA FORMATO HORIZONTAL EN TODAS tus respuestas que muestren horarios. SIN EXCEPCIONES.
        - Usa formato compacto (separado por comas en una sola línea).
        - NUNCA uses viñetas/bullets (-, •) para mostrar los horarios.
        - FORMATO CORRECTO: "Horarios: 08:30, 08:45, 09:00, 09:15, 09:30, 09:45, 10:00, 10:15..."
        - FORMATO INCORRECTO: "- 08:30\n- 08:45\n- 09:00" (ESTO ESTÁ PROHIBIDO)
        - PROHIBIDO resumir (ej: no digas "de 9 a 11"). 
        - PROHIBIDO redondear a horas cerradas.

        ⚠️ REGLA DE AGENDAMIENTO (OBLIGATORIO):
        - Si el usuario confirma un horario y ya tienes sus datos, DEBES incluir el bloque de METADATA al final de tu respuesta.
        - Ejemplo: "[METADATA: { \"action\": \"create_appointment\", \"time\": \"09:00\", \"date\": \"2026-02-14\", ... }]"
        - Si no incluyes el bloque METADATA, la cita NO se guardará en la base de datos.
        ...
        try {
            ...
            if (slots.length > 0 && targetDate && targetDate !== '9999-99-99') {
                ...
                let securityInstruction = `🔴 INSTRUCCIÓN DE SEGURIDAD DEL SISTEMA (MANDATORIA):
                    HE DETECTADO QUE HAY ${slots.length} HORARIOS DISPONIBLES PARA EL ${targetDate} (${dayName.toUpperCase()}).`;

                if (dayOfWeek === 6) {
                    securityInstruction += `\n\n⚠️ REGLA CRÍTICA SÁBADO: HOY ES SÁBADO. CERRAMOS A LAS 15:00 Y NO HAY ALMUERZO. SI HAY CUPOS A LAS 14:00 O 14:30, SON VÁLIDOS.`;
                } else if (dayOfWeek >= 1 && dayOfWeek <= 5) {
                    securityInstruction += `\n\n⚠️ REGLA CRÍTICA SEMANA: HOY ES ${dayName.toUpperCase()}. CERRAMOS DE 13:00 A 15:00 POR ALMUERZO. ESTÁ PROHIBIDO AGENDAR EN ESE RANGO.`;
                }
                ...
                securityInstruction += `\n\nTU TAREA ES: OFRECER ESTOS HORARIOS AHORA MISMO SIN FILTRAR NINGUNO.
                    ...
                    - Resumir horarios (ej: "de 9 a 11"). DEBES MOSTRAR CADA HORARIO DE LA LISTA.
                    - Redondear a horas cerradas si hay minutos intermedios.
                    - USAR LISTAS VERTICALES. USA FORMATO HORIZONTAL (Comas).
                    - Olvidar el bloque [METADATA: ...] al confirmar una cita. SIN METADATA NO HAY CITA.

                    HORARIOS REALES DISPONIBLES: ${slots.join(', ')}

                    SI EL USUARIO PIDE UN HORARIO QUE ESTÁ EN ESTA LISTA, DEBES DECIR QUE SÍ Y PROCEDER.`;
            ...
            if (!bookingExecuted && metadata && metadata.action === 'create_appointment') {
                const mName = metadata.name || session.name;
                const mCedula = metadata.cedula || session.cedula;
                const mAge = parseInt(metadata.age || session.age);
                const mIsPediatric = botResponse.toLowerCase().includes('niño') || botResponse.toLowerCase().includes('infantil') || botResponse.toLowerCase().includes('hijo') || (mAge > 0 && mAge < 12);
