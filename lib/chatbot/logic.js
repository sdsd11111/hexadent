...
export async function processChatbotMessage(phoneNumber, messageText) {
    console.log(`[Chatbot Logic] Starting OpenAI flow for ${phoneNumber}`);
    ...
    try {
        ...
        const response = await openai.chat.completions.create({
            ...
            messages: [
                ...
                { role: "user", content: `Mensaje de WhatsApp del número ${phoneNumber}: ${messageText}` }
            ],
            temperature: 0.7,
        });

        const botResponse = response.choices[0].message.content;
        console.log(`[Chatbot Logic] OpenAI Response generated successfully.`);
