...
export async function processChatbotMessage(phoneNumber, messageData, clinicNumber = null) {
    ...
    try {
        ...
        if (isExplicitBooking || isCurrentlyInBookingFlow || resolvedDate) {
            ...
            if (hasActiveMotive && targetDate) {
                try {
                    ...
                    if (isSunday) {
                        availabilityContext = `[ESTADO CRÍTICO: CERRADO para ${targetDate}. Motivo: Los DOMINGOS la clínica Hexadent permanece cerrada por descanso. NO HAY HORARIOS DISPONIBLES. Dile al usuario amablemente que el ${dayName} no trabajamos y ofrece agendar para otro día de la semana.]`;
                    } else {
                        session.target_date = targetDate;
                        session.duration = duration;
                        session.current_flow = "Motivo recibido";
                        ...
                        if (isBlockedRows.length > 0) {
                            availabilityContext = `[ESTADO: CERRADO para ${targetDate}. Motivo: ${isBlockedRows[0].reason || 'Cerrado'}. Ofrece agendar para otro día.]`;
        ...
        const systemPrompt = `
            ...
            CUIDADO CON LOS EMOJIS: Añade un espacio DESPUÉS de cada emoji (ej: "😊 "). No envíes nunca el carácter "?" al final de una frase que no sea pregunta ni el carácter de reemplazo .
