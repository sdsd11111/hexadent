import { processChatbotMessage, ensureTables } from './logic';
import db from '../db';
...
export async function debounceMessage(phoneNumber, text, clinicNumber) {
    ...
    try {
        ...
        // 4. Fetch all buffered messages
        const [rows] = await db.execute(
            'SELECT message FROM chatbot_buffer WHERE phone = ? ORDER BY id ASC',
            [cleanPhone]
        );

        // 5. Clean up buffer and lock IMMEDIATELY after fetching
        // This prevents re-processing if the processChatbotMessage takes a long time
        await db.execute('DELETE FROM chatbot_buffer WHERE phone = ?', [cleanPhone]);
