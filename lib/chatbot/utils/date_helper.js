...
export const dayMapEcu = {
    'lunes': 1, 'luines': 1, 'ellluines': 1, 'elluines': 1, 'el lunes': 1, 'martes': 2, 'miércoles': 3, 'miercoles': 3, 'jueves': 4, 'viernes': 5, 'sábado': 6, 'sabado': 6, 'domingo': 0
};
...
export function resolveDate(text, now) {
    ...
    if (fullDateMatch) {
        ...
        const resDate = new Date(yearNum, monthIdx, dayNum);

        // If the date passed already this year and no year was specified, assume next year
        if (!fullDateMatch[3] && resDate < anchor && (anchor.getTime() - resDate.getTime() > 24 * 60 * 60 * 1000)) {
            resDate.setFullYear(yearNum + 1);
        }
        const y = resDate.getFullYear();
        const m = (resDate.getMonth() + 1).toString().padStart(2, '0');
        const d = resDate.getDate().toString().padStart(2, '0');
        return `${y}-${m}-${d}`;
    }
    ...
    // 4. Just a number (e.g., "el 15") - pero NO si es seguido de "y" + numero (hora) o palabras de hora
    const justNumMatch = text.match(/(?:el\s+)?(\b\d{1,2}\b)(?!\s+(?:de|y\s*\d+|años?|mes(?:es)?|vueltas|velitas|kilos|kg|gr|am|pm|a\.m\.|p\.m\.|horas?|minutos?))/i);
    const lower = text.toLowerCase();
    const isAgePhrase = lower.includes('años') || lower.includes('año') || lower.includes('edad') || lower.includes('tengo') || lower.includes('soy') || lower.includes('nac');

    if (justNumMatch && !isAgePhrase) {
