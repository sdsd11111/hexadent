...
export function resolveDate(text, now) {
    ...
    return date.toISOString().split('T')[0]; // Default to today if nothing clear is found
}
...
export function getDateTruth(targetDateStr, now) {
    ...
    try {
        ...
        const isPossible = dateObj >= earliest;

        return `[VERDAD DE FECHA: El ${targetDateStr} es ${dayName}. Regla de 24h: ${isPossible ? 'CUMPLIDA' : 'INCUMPLIDA (Muy pronto)'}. Laborable: ${isSunday ? 'NO (Cerrado)' : 'SÍ'}.]`;
    } catch (e) {
